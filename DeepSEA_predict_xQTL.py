...
def parse_args():
    parser = argparse.ArgumentParser(description="DeepSEA classifiers (XGB/RF/L1/ElasticNet/SVM) with chromosome-based train/test split and train-only chromosome tuning CV; train or predict. Model-specific hyperparameters come from internal defaults plus auto-tune outputs.")
    ...
    parser.add_argument("--model", type=str, choices=["xgb", "rf", "l1", "enet", "svm"], default="xgb")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Saved model directory (for --mode predict): model_meta_{mt}.json, model_{mt}.pkl|json, scaler_{mt}.pkl (xgb/l1/enet/svm). Use --model matching training.",
    )
    parser.add_argument(
        "--predict_out",
        type=str,
...
def main():
    ...
    if args.model == "rf":
        ...
        args.svm_kernel = tuned.get("svm_kernel", args.svm_kernel)
    ...
    if args.model == "xgb":
        ...
        if args.svm_kernel != "linear":
            svm_kwargs["gamma"] = args.svm_gamma
