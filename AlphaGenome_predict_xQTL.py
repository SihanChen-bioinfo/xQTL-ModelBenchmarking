...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--model",
        type=str,
        choices=["rf", "xgb", "l1", "enet", "svm"],
        default="rf",
        help="Model type: rf, xgb, l1 (L1 logistic), enet (elastic-net logistic), svm (linear SVC with probabilities).",
    )
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Model directory for --mode predict: model_meta_{mt}.json, model_{mt}.json|pkl, scaler_{mt}.pkl (xgb/l1/enet/svm). Use --model matching training.",
    )
    p.add_argument(
        "--predict_out",
        type=str,
    ...
    for cand in samples:
        ...
        for tr_idx, te_idx in tune_folds:
            ...
            if args.model == "xgb":
                ...
                svm_kwargs = {
                    "C": float(cand["lr_c"]),
                    "kernel": cand["svm_kernel"],
                    "probability": True,
                    "class_weight": "balanced",
                    "random_state": args.random_state,
                }
                if cand["svm_kernel"] != "linear":
                    svm_kwargs["gamma"] = cand["svm_gamma"]
...
def main():
    ...
    if args.mode == "predict":
        ...
        if not os.path.isfile(meta_path):
            print(
                f"ERROR: missing {meta_path} (use --model matching training; expected model_meta_{{rf|xgb|l1|enet|svm}}.json).",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if args.model == "xgb":
        ...
        if scaler is not None and args.model in {"l1", "enet", "svm"}:
            with open(os.path.join(args.out_dir, _out_name("scaler.pkl", mt)), "wb") as f:
                pickle.dump(scaler, f)
